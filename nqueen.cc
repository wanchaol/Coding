...
class Solution {
   private:
    ...
    bool IsSafe(int row, int col, vector<bool> &colempty,
                vector<bool> &updiagempty, vector<bool> &downdiagempty) {
        return colempty[col] && updiagempty[row + col] &&
               downdiagempty[boardsize - 1 + row - col];
    }

    void PlaceQueen(vector<string> &queenOnBoard, int row, int col,
                    vector<bool> &colempty, vector<bool> &updiagempty,
                    vector<bool> &downdiagempty) {
        queenOnBoard[row][col] = 'Q';
        colempty[col] = false;
        updiagempty[row + col] = false;
        downdiagempty[boardsize - 1 + row - col] = false;
    }
    void RemoveQueen(vector<string> &queenOnBoard, int row, int col,
                     vector<bool> &colempty, vector<bool> &updiagempty,
                     vector<bool> &downdiagempty) {
    ...
    void Solver(vector<string> &queenOnBoard, vector<bool> &colempty,
                vector<bool> &updiagempty, vector<bool> &downdiagempty, int row,
        ...
        for (int col = 0; col < boardsize; ++col) {
            if (IsSafe(row, col, colempty, updiagempty, downdiagempty)) {
                PlaceQueen(queenOnBoard, row, col, colempty, updiagempty,
                           downdiagempty);
                Solver(queenOnBoard, colempty, updiagempty, downdiagempty,
                       row + 1, res);
                RemoveQueen(queenOnBoard, row, col, colempty, updiagempty,
                            downdiagempty);
            }
        }
    }
