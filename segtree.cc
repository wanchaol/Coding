...
int SegTree::RecursiveBuild(int input[], int start, int end, int index) {
	...
	vals[index] = RecursiveBuild(input, start, mid, 2*index + 1) + RecursiveBuild(input, mid + 1, end, 2*index + 2);
	return vals[index];
...
int SegTree::RecursiveFind(int s, int e, int start, int end, int index) {
	...
	return RecursiveFind(s, mid, start, end, 2 * index + 1) + RecursiveFind(mid + 1, end, start, end, 2 * index + 2);
