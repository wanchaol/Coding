...
bool matchcode(string message, string code) {
    ...
    while (iss >> word) {
        if (mmap[word] != cmap[code[index - 1]]) return false;
...
bool backtrack(string message, string code, vector<string> &partitions,
               int begin) {
...
int main() {
    // vector<string> messages{"red green blue","red blue blue", "red blue
    // blue", "red green blue",  "red green blue", "red blue blue" };
    ...
    vector<string> messages{"redgreenblue", "redgreenblue", "redbluered",
                            "redgreenr"};
